...
/*   Updated: 2024/11/11 23:58:25 by ebouboul         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_floor_ceiling(t_mlx *mlx, int ray, int t_pix, int b_pix)
{
	...
	i = b_pix;
	c = get_rgba(ft_atoi(mlx->data->ff[0]), ft_atoi(mlx->data->ff[1]), \
		ft_atoi(mlx->data->ff[2]), 255);
	while (i < S_H)
		my_mlx_pixel_put(mlx, ray, i++, c);
	c = get_rgba(ft_atoi(mlx->data->cc[0]), ft_atoi(mlx->data->cc[1]), \
